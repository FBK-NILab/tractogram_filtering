    ...
    with torch.no_grad():
        ...
        #while sls_count <= len(dataset):
            ...
            if cfg['with_gt'] and consumed:
                ...
                mae = torch.mean(abs(obj_target.data.float64().cpu() - obj_pred_choice.data.float64().cpu())).float().item()
                mse = torch.mean((obj_target.data.float64().cpu() - obj_pred_choice.data.float64().cpu())**2).float().item()
