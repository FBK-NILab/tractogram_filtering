...
from models import (DEC, NNC, BiLSTM, DECSeq, DECSeqGlob, GCNConvNet,
                    GCNConvNetBN, PNptg2, GAT)
...
def train(cfg):
    ...
    trans_val = []
    if cfg['rnd_sampling']:
        trans_train.append(ds.RndSampling(sample_size, maintain_prop=False, prop_vector=[1,1]))
...
def dump_code(cfg, logdir):
    ...
    if not os.path.exists(codedir):
        os.makedirs(codedir)
    source_code_list = './*.py'
    os.system('cp %s %s' % (source_code_list, codedir))
