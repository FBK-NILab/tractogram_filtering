    ...
    with torch.no_grad():
        ...
        while j < len(dataset):
            ...
            if cfg['with_gt'] and consumed:
                ...
                np.save(data['dir']+'/y_pred_decseq5_resampled16_correctedges',y_pred)
                y_test = obj_target.cpu().numpy()
                np.save(data['dir']+'/y_test_decseq5_resampled16_correctedges',y_test)
                np.save(data['dir']+'/streamlines_decseq5_resampled16_correctedges',streamlines)
